...
戍式式 Time-Travel_SOKOBAN.fsproj    # .NET 10 F# project file
戍式式 run.bat
戍式式 run.sh
戍式式 Program.fs                  # Entry point, file save
戍式式 Mapdef.fs                   # Defining Types for needs
戍式式 Reader.fs                   # Read map file and pass map data
戍式式 Setting.fs                  # Setting values by immutable variables
戍式式 running.fs                  # Program running by loop
戍式式 Viewer.fs                   # Printing maps
戍式式 Mainscreen.fs               # Printing menu
戍式式 README.md
戍式式 requirements.md
戌式式 maps/
    戌式式 level1.txt      # Map data as txt file, for 1-10.
```
...
|--------|---------------|
| `Types` | Immutable board operations: `create`, `tryPlace`, `winner`, `isFull`, `render` |
| `Reader`  | Main game loop, input validation, random enemy move, end-state display |
| `Setting` | Entry point; prints welcome message, drives play-again loop |
| `Logic` | Entry point; prints welcome message, drives play-again loop |
| `Viewer` | Entry point; prints welcome message, drives play-again loop |
| `Mainscreen` | Entry point; prints welcome message, drives play-again loop |
